...
def train(train_file, validation_file, model_file, epochs, batch_size):
    ...
    if path.isfile(model_file):
        ...
        max_i = max_index(train_file[:train_file.rfind('-')] + '-indices.h5')
        model = cnn.create_model(smiles_matrix.shape[1:], classes.shape[1], max_i)
...
def max_index(index_file):
    index_hdf5 = h5py.File(index_file, 'r')
    max = index_hdf5['index'].shape[0] - 1
    index_hdf5.close()
    return max
